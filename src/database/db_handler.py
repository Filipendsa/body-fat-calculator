...
class DatabaseHandler:
    ...
    def seed_food_database(self):
        ...
        # 1. Lista EXCLUSIVA fornecida por você
        foods_data = [
            # PROTEÍNAS ANIMAIS (Levítico 11)
            ...
            ("Pão de Forma Integral", "Carbo", "fat", 2, 118, 4.6, 20, 2.2, "vegan,vegetariano,onivoro"),
            ...
            ("Laranja Pera", "Fruta", "un", 1, 65, 1.2, 16, 0.1, "vegan,frugivoro"),
            ("Mamão Papaia", "Fruta", "un", 0.5, 60, 0.8, 15, 0.2, "vegan,frugivoro"),
            ("Abacaxi", "Fruta", "fat", 1, 50, 0.5, 13, 0.1, "vegan,frugivoro"),
            ("Melão", "Fruta", "fat", 1, 30, 0.8, 8, 0, "vegan,frugivoro"),
            ("Melancia", "Fruta", "fat", 1, 30, 0.6, 8, 0, "vegan,frugivoro"),
            ("Morango", "Fruta", "un", 5, 20, 0.5, 5, 0, "vegan,frugivoro"),
            ("Uva", "Fruta", "un", 10, 35, 0.4, 9, 0, "vegan,frugivoro"),
            ...
            ("Requeijão Light", "Gordura", "g", 10, 18, 1.3, 0.3, 0.8, "lactovegetariano,onivoro"),
        ...
        # 2. Limpa TUDO antes de inserir para garantir que só existam esses itens
        cursor.execute("DELETE FROM foods")
        
        # 3. Insere a lista limpa
        cursor.executemany("""
            INSERT INTO foods 
            (name, category, unit, base_qty, kcal, prot, carb, fat, tags) 
            VALUES (?,?,?,?,?,?,?,?,?)
        """, foods_data)
        
        self.conn.commit()
        print(f"Banco de dados resetado com {len(foods_data)} alimentos limpos.")
    ...
    def cleanup_duplicates(self):
        ...
        # 1. Remove alimentos com nomes duplicados na tabela 'foods', mantendo apenas o ID menor
        cursor.execute("""
            DELETE FROM foods 
            WHERE id NOT IN (
                SELECT MIN(id) FROM foods GROUP BY name, unit, base_qty
            )
        """)
        
        # 2. Remove entradas idênticas no log de dieta (mesmo alimento na mesma refeição)
        # Isso limpa o que aparece duplicado na imagem image_c16dfa
        cursor.execute("""
            DELETE FROM diet_log 
            WHERE rowid NOT IN (
                SELECT MIN(rowid) 
                FROM diet_log 
                GROUP BY user_id, meal_name, food_id, quantity
            )
        """)
